...
**任何有副作用的操作，执行前必须存在对应的 GitHub Issue，执行结果必须在 Issue 或 PR 中有记录。**
...
| 操作类型 | 审批要求 |
|--------|--------|
| 日常任务（bug 修复、code review、merge to develop） | 自主执行，Issue 记录即可 |
| 重大变更（架构改动、新技术引入、大规模重构） | CTO 审批 |
| 生产环境部署 | Owner 审批 |
...
1. 描述问题和拟执行操作
2. 日常修复可自主执行；生产部署需 Owner 审批
3. 立即在 Issue 中记录操作结果
4. 24 小时内补开正式 Issue（若紧急处理前无时间开）
