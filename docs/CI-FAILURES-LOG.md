...
| 字段 | 内容 |
|------|------|
| **ID** | CI-2026-03-20-004 |
| **Repo** | Backend |
| **Workflow** | `cd-test.yml` |
| **触发场景** | develop push — Deploy → Test job 失败，Health check 连接 port 9001 超时 |
| **状态** | ⏳ 进行中 |

**根本原因**
`nordhjem_medusa_test` 容器处于 crash loop（`Restarting (1) ...`）。容器日志显示：

```
Error: Cannot find module '/app/.medusa/server/src/instrument.js'
```

`instrument.js` 是 Sentry 集成文件（`src/instrument.ts`），编译后应出现在 `.medusa/server/src/instrument.js`。但 test 环境的 Docker 镜像中该文件缺失，导致 Node.js preload 阶段直接崩溃，Medusa 无法启动。

**影响范围**
- CD-Test 的 Deploy → Test、E2E → Test、Smoke Test → Test 全部失败
- Promote → Staging 被跳过（因 smoke-test 失败）
- develop → staging 自动 promote 链路完全中断

**修复方案**
需要排查：
1. `medusa.config.ts` 中是否配置了 `--require src/instrument.ts` 的 preload
2. Docker build 过程中 `src/instrument.ts` 是否被正确编译到 `.medusa/server/`
3. 如果 Sentry DSN 未配置，`instrument.ts` 是否应该 graceful skip 而非 crash

**经验教训**
- 新增 preload 模块（如 Sentry instrument）时，必须确保 Docker build 流程包含该文件的编译输出
- Preload 失败会导致整个 Node.js 进程无法启动 — preload 模块必须有 fallback/try-catch 机制
- Test 环境的健康状态应纳入日常监控，不应等到 CI 失败才发现
...
| 字段 | 内容 |
|------|------|
| **ID** | CI-2026-03-20-005 |
| **Repo** | Backend |
| **Workflow** | `deploy-ops-scripts.yml` |
| **触发场景** | develop push 持续触发（06:13 UTC 3 次, 05:27 UTC 1 次），workflow 无法执行 |
| **状态** | ⏳ 进行中 |

**根本原因**
与 CI-2026-03-19-003 同源但有新情况。该文件**不存在于 main 分支**（仅在 develop 上），但 `on: push: branches: [main]` 配置无法阻止 develop push 触发。

CI-2026-03-19-003 中添加的 `if: github.ref == 'refs/heads/main'` job guard 似乎未被合入 develop 分支（或该修复在另一个 PR 中）。当前 develop 上的 `deploy-ops-scripts.yml` 没有 job-level guard，每次 push 都会尝试执行并因无 production environment access 而失败。

**修复方案**
在 `deploy-ops-scripts.yml` 的 `deploy` job 中添加 guard：
```yaml
jobs:
  deploy:
    if: github.ref == 'refs/heads/main'
...
**优先级**: P1 — 每次 develop push 都产生一个失败 run，噪音严重
...
| 字段 | 内容 |
|------|------|
| **ID** | CI-2026-03-20-006 |
| **Repo** | Backend |
| **Workflow** | `issues-daily-check.yml` (Issue Management) |
| **触发场景** | main 分支 cron 触发（UTC 08:00）+ workflow_dispatch |
| **状态** | ⏳ 进行中 |

**根本原因**
`issues-daily-check.yml` 使用 `actions/github-script@v8`，脚本中 `require('@octokit/rest')` 失败：

```
Error: Cannot find module '@octokit/rest'
```

`actions/github-script@v8` 不再内置 `@octokit/rest` 模块的独立 `require` 路径。v7 中 `require('@octokit/rest')` 可用，v8 中已移除。该 workflow 需要 `@octokit/rest` 来创建使用 CTO_PAT 的独立 Octokit 实例。

**修复方案**
两种选择：
1. **降级到 v7**（推荐，与项目标准一致）：`actions/github-script@v7`
2. **改写代码**：用 `const { Octokit } = require('@octokit/core')` 或直接用 `github` 对象 + token 替换

**优先级**: P2 — 仅影响每日 Issue 看板检查，不阻塞代码合并
...
| 字段 | 内容 |
|------|------|
| **ID** | CI-2026-03-20-007 |
| **Repo** | Backend |
| **Workflow** | `pr-automation.yml` (PR Automation) |
| **触发场景** | main 分支持续失败（07:53-07:54 UTC 3 次，06:04-06:06 UTC 3 次） |
| **状态** | ⏳ 进行中（CI-2026-03-19-004 同源，main 未同步修复） |

**根本原因**
与 CI-2026-03-19-004/CI-2026-03-20-003 完全同源。develop 分支的 `pr-automation.yml` 已修复（PR #661），但修复尚未通过 staging → main 同步到 main 分支。main 上仍运行旧版 workflow，`check_suite` 触发器持续产生失败。

**修复方案**
需要将 develop 上的修复通过正常 promote 流程（develop → staging → main）同步到 main。或者作为 hotfix 单独合入 main。
...
| OI-008 | 修复 test 容器 `instrument.js` 缺失导致 crash loop（CI-2026-03-20-004） | **P1** | Backend + DevOps |
| OI-009 | `deploy-ops-scripts.yml` 添加 `if: github.ref == 'refs/heads/main'` guard（CI-2026-03-20-005） | **P1** | DevOps |
| OI-010 | `issues-daily-check.yml` 降级 `github-script@v8` → `@v7`（CI-2026-03-20-006） | P2 | DevOps |
| OI-011 | 全局扫描并统一所有 workflow 的 `github-script` 版本到 `@v7`（cd-test, cd-staging, cd-production, issues-daily-check 仍用 v8） | P2 | DevOps |
...
| 13 | **Preload 模块（`--require`）必须有 try-catch，否则缺失时整个进程无法启动** | CI-2026-03-20-004 |
| 14 | **`actions/github-script@v8` 不支持 `require('@octokit/rest')`，锁定 v7** | CI-2026-03-20-006 |
| 15 | **develop 修复必须尽快 promote 到 main，否则 main 上旧 workflow 持续产生噪音** | CI-2026-03-20-007 |
...
| 2026-03-20 | v1.2 | 新增 4 个故障记录：CD-Test crash loop (instrument.js)、deploy-ops-scripts 无 guard、issues-daily-check v8 不兼容、PR Automation main 未同步 | DevOps Lead |
