...
| 部署平台  | Railway                             | Test/Staging/Production 环境托管与发布      |
...
- 合并后触发对应环境部署到 Railway。
- 若发布异常，通过回滚到前一稳定镜像/提交恢复服务。

### 5.3 Docker 镜像构建流程

1. 使用 `Dockerfile` 基于 Node 20 构建运行镜像。
2. `npm ci` 安装依赖，`npx medusa build` 生成后端构建产物。
3. CI 将镜像推送到镜像仓库（如 GHCR）。
4. Railway 拉取指定镜像标签并执行滚动更新。
